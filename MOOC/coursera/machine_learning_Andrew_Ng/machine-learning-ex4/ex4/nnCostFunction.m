...
a1 = [ones(m, 1), X];  % size(m, n+1)
z2 = Theta1 * a1';     % size(n_hidden, m)
a2 = sigmoid(z2);
a2 = [ones(1, m); a2];   % size(n_hidden + 1, m)
z3 = Theta2 * a2;      % size(n_output, m)
a3 = sigmoid(z3);

% encoding y labels
y_cat = zeros(num_labels, m);
for i=1:m
  y_cat(y(i), i) = 1;
end;
